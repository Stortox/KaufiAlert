...
class _OfferDetailState extends State<OfferDetail> {
  ...
  void initState() {
    ...
    if (widget.product.gtin.isNotEmpty) {
      http
          .get(
            Uri.parse(
              'https://world.openfoodfacts.org/api/v2/product/${widget.product.gtin}.json',
            ),
          )
          .then((response) {
            if (response.statusCode == 200) {
              ...
              setState(() {
                ingredientsText =
                    data['product']['ingredients_text_with_allergens_de'] ?? '';
              });
            }
          });
        ...
        child: Center(
          child: Column(
            children: [
              ...
              if (widget.product.gtin.isNotEmpty)
                FutureBuilder(
                  future: getOpenFoodFactsGtin(widget.product.gtin),
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      final nutrients =
                          snapshot.data as List<MapEntry<String, dynamic>>;
                      return Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 8.0),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Future<List<MapEntry<String, dynamic>>> getOpenFoodFactsGtin(
    String gtin,
  ) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var nutrient in nutrients.entries) {
        if (defaultNutrients.contains(
          nutrient.key.toString().substring(
            0,
            nutrient.key.toString().contains("-")
                ? nutrient.key.toString().indexOf("-")
                : nutrient.key.toString().contains("_")
                ? nutrient.key.toString().indexOf("_")
                : nutrient.key.toString().length,
          ),
        )) {
          if (nutrient.key.toString().contains("energy") &&
              !nutrient.key.toString().contains("kcal")) {
            continue;
          }
          if (!nutrient.key.toString().contains("-") &&
              !nutrient.key.toString().contains("_")) {
            nutrientNames.add(
              nutrient.key.toString()[0].toUpperCase() +
                  nutrient.key.toString().substring(1),
            );
            ...
            nutrientNames.add(
              nutrient.key.toString()[0].toUpperCase() +
                  nutrient.key.toString().substring(
                    1,
                    nutrient.key.toString().indexOf("-kcal"),
                  ),
            );
            ...
            if (nutrient.value.toString().contains(".")) {
              if (nutrient.value
                      .toString()
                      .substring(nutrient.value.toString().indexOf(".") + 1)
                      .length >
                  3) {
                nutrientValues.add(
                  nutrient.value.toString().substring(
                    0,
                    (nutrient.value.toString().indexOf(".") + 3),
                  ),
                );
              } else {
              ...
              nutrientValues.add(
                nutrient.value.toString().substring(
                  0,
                  nutrient.value.toString().length > 4
                      ? 4
                      : nutrient.value.toString().length,
                ),
              );
            }
      ...
      if (!(nutrientNames.length > nutrientUnit.length) &&
          !(nutrientNames.length > nutrientValues.length)) {
        nutrientsList.addAll(
          nutrientNames.map(
            (name) => MapEntry(
              name,
              '${nutrientValues[nutrientNames.indexOf(name)]} ${nutrientUnit[nutrientNames.indexOf(name)]}',
            ),
          ),
        );
      }
